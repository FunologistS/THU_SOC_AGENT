...
除主检索词（topic）外，可增加**副检索词**，用于缩写、同义词等，与主词按「或」或「且」组合：

- **`--terms`**：多个检索词，逗号或换行分隔，如 `electric vehicles,EVs,BEV`。
- **`--logic`**：词间逻辑，与 `--terms` 配合；若为 `or`（默认），则命中任一词即保留；若为 `and`，则需同时命中所有词。

示例（主词 + 缩写，用「或」扩大命中；`--terms` 中需包含主词或单独用 topic 作主词，UI 会传主词 + 副检索词）：

    node .claude/skills/journal-search/scripts/run.mjs electric_vehicles --terms "electric vehicles,EVs,BEV" --logic "or"

系统会对每个词做**按词匹配**（同根、单复数、轻量纠错），主词与副词逻辑一致。
...
- **检索的分区**：本次参与检索的期刊在数据源中出现的分区并集（Q1、Q2、Q3、Q4）。若你只选了某学科 Q1，仍可能看到多种分区，因为同一刊可归属多个 WOS 学科，各学科分区不同；筛选逻辑是按「该学科下 Q1」选刊，但每刊在 yml 中只存一个 quartile，故表头会列出所有出现过的分区。  
