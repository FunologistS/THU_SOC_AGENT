...
**表格规范**：该主题的论文、附录论文卡片均用 Markdown 表格呈现；附录表使用「引用与标题」一列（可点击引用 + 标题同格，便于换行与自适应），该有表就画表。
...
**（一）该主题的主要内容**
1、 第一句…（句末用可点击引用）[(姓氏 & 姓氏, Year)](#paper-<id>)；[(姓氏 et al., Year)](#paper-<id>)。链接外不要再加括号。本小节用2-4句话概括。
2、 第二句…
3、 …

**（二）主要的视角/视角框架**
1、 视角一… [(Smith, 2025)](#paper-3)；[(Chen & Wang, 2026)](#paper-4)。不要写（[(…)](#paper-id)）这种双层括号。本小节2-5个要点。
2、 视角二…
3、 …

**（三）重复的主张/结论**
1、 …[(Smith, 2025)](#paper-1)；[(Padovani & Pavan, 2016)](#paper-2)。不得写无引用的“研究指出……”。本小节2-5个要点。
2、 …
3、 …

**（四）从摘要中可见的限制**
1、 …用可点击引用标注 [(Author, Year)](#paper-<id>)。采用软性表述：“通常”“倾向于”“摘要中很少提到……”。本小节2-5个要点。
2、 …
3、 …

**（五）研究空白/下一步研究方向**
1、 若某条基于某篇文献的局限或建议，用可点击引用标注 [(Author, Year)](#paper-<id>)。本小节2-5个要点。
2、 …
3、 …
...
| 序号 | 引用 | 一句话总结 |
|:---:|---|:---|
| 1 | [(文内引用)](#paper-<id>) | <基于卡片的一句总结，不超过35字> |
| 2 | … | … |

${wantAppendix ? `
### 附录：论文卡片

附录表为四列：「引用与标题」合并为一列（同一单元格内写可点击引用与标题，便于 UI 换行），问题、方法、主要主张各一列。表头如下：

| 引用与标题 | 问题 | 方法 | 主要主张 |
|:---|:---|:---|:---|
| [(文内引用)](#paper-<id>) **<标题>** (<年份>) — <期刊> | … | … | … |
` : ""}
...
- **层级（强制）**：小节标题用「（一）（二）（三）（四）（五）（六）」；每小节要点用 1、2、3、 编号；若有下一层用（1）（2）（3）。不得用无编号的 - 列表。
- **表格（强制）**：「该主题的论文」与「附录：论文卡片」必须用 Markdown 表格输出；附录表头为「引用与标题 | 问题 | 方法 | 主要主张」，第一列内写可点击链接紧跟标题（同一格），便于自适应与换行。
...
- 表格「引用」列必须为可点击链接：[(文内引用)](#paper-<id>)，<id> 为卡片中的 ID（数字）。
...
function addHeadingNumber(content, idx) {
  ...
  for (let i = 0; i < lines.length; i++) {
    if (/^##\s+/.test(lines[i])) {
      ...
      // 若模型已自行加了“第X部分：”“一、”之类前缀，则不重复加
      if (/^[一二三四五六七八九十]+、/.test(title)) break;
      lines[i] = `## ${num}、${title}`;
      break;
    }
  }
...
for (let i = 0; i < toProcess.length; i++) {
  ...
  for (let b = 0; b < batches.length; b++) {
    ...
    console.log(`[concept_synthesis] Cluster ${cid} 第 ${b + 1}/${batches.length} 批 (${cards.length} papers) -> LLM...`);
    ...
    if (content == null || String(content).trim().length === 0) {
      console.error(`[concept_synthesis] Cluster ${cid} 批 ${b + 1} 返回为空`);
      clusterParts.push(`（批 ${b + 1} 调用失败或返回为空）`);
    } else {
      clusterParts.push(ensureSubsectionLineBreaks(content));
    }
  }

  if (clusterParts.length === 0 || clusterParts.every((p) => /^（批.*/.test(p.trim()))) {
    briefingParts.push(`## Cluster ${cid}\n\n（调用失败，请检查 API 或重试）`);
  } else {
    const combined = clusterParts.filter((p) => !/^（批.*/.test(p.trim())).join("\n\n");
