#!/usr/bin/env node
/**
 * 6_export_to_md.mjs — 主流程第 6 步（可选）：导出为 Markdown，方便查看与传播
 *
 * Subcommands:
 *   1) journals : references/system/journals.yml -> outputs/system/journal-catalog/journals_*.md + journals_latest.md
 *   2) jcr      : normalized JCR CSV -> outputs/system/journal-catalog/*_report_vN.md
 *
 * Usage:
 *   node .claude/skills/journal-catalog/scripts/6_export_to_md.mjs journals
 *
 *   node .../6_export_to_md.mjs jcr [normalized_csv] [out_md]
...
function die(msg, code = 1) {
  console.error(`[6_export_to_md] ERROR: ${msg}`);
  process.exit(code);
}
...
function exportJournals() {
  const inputPath = path.join(__dirname, "../references/system/journals.yml");
  if (!fs.existsSync(inputPath)) die(`journals.yml not found: ${inputPath}`);

  const data = yaml.load(fs.readFileSync(inputPath, "utf8"));
  const journals = data?.journals || [];

  const header = "| short | name | issn | eissn | 分区(学科) | site | notes |";
  const sep = "|-------|------|------|--------|------------|------|-------|";
  const quartileByDiscipline = (j) =>
    Array.isArray(j.source_categories) && j.source_categories.length
      ? j.source_categories.join("; ")
      : (j.quartile ?? "");
  const rows = journals.map(
    (j) =>
      `| ${escapeCell(j.short)} | ${escapeCell(j.name)} | ${escapeCell(j.issn)} | ${escapeCell(
        j.eissn
      )} | ${escapeCell(quartileByDiscipline(j))} | ${escapeCell(j.site)} | ${escapeCell(j.notes)} |`
  );
