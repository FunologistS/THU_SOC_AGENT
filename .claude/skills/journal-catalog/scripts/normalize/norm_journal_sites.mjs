 ...
 *
 * Usage
 *   node .../norm_journal_sites.mjs.mjs
 *   node .../norm_journal_sites.mjs.mjs --in path/to/journals.yml
 *   node .../norm_journal_sites.mjs.mjs --dry-run
 *   node .../norm_journal_sites.mjs.mjs --fix-oup-root
 *   node .../norm_journal_sites.mjs.mjs --delay-ms 150
 */
...
async function main() {
  ...
  if (!ARGS.dryRun) {
    ...
    console.log(`\n[norm_journal_sites.mjs] wrote back = ${inputPath}`);
  } else {
    console.log(`\n[norm_journal_sites.mjs] dry-run (no write back)`);
  }

  writeYaml(snapshotPath, out);
  console.log(`[norm_journal_sites.mjs] wrote snapshot = ${snapshotPath}`);
  console.log(
    `[norm_journal_sites.mjs] summary: checked=${journals.length} changed=${changed} nonEmptySite=${nonEmptySite} fixOupRoot=${fixedOupRoot}`
  );
}
