...
function loadStyleSamples(files = [], maxTotalChars = 1000) {
  ...
  for (const name of files) {
    ...
    const fullPath = path.join(REFERENCES_ACADEMIC, name);
...
5）保持学术克制，不要夸张修辞；
6）**必须保留文内引用**：原文中的 (Author & Author, Year) 或 (Author et al., Year) 以及可点击链接 [文本](#paper-数字) 必须完整保留，不得删除或改写成无引用表述；基于某篇/某几篇文献的论断在改写后仍须带对应引用，做到有理有据。`;
...
5）**保留所有文内引用**：合并时不得删除 (Author, Year) 或 [文本](#paper-数字) 形式的引用与链接，综述中基于文献的论述须继续带引用。`;
...
async function doMergeAndOptionalSmooth({ client, styleSamples, mergedDraft, outputFile, outputDir }) {
...
async function main() {
  ...
  const styleSamples = loadStyleSamples(styleFiles, MAX_STYLE_CHARS);
  if (!styleSamples && fs.existsSync(REFERENCES_ACADEMIC)) {
    console.warn(
      "[writing_under_style] 未加载到任何风格样本（检查 references/academic 下是否有 academic-2a-tsyzm.md、academic-2b-qnyj.md）"
    );
  }
