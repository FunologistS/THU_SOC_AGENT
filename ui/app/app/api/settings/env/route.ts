...
/** 各 skill 用到的环境变量（仅用于展示是否已配置，不返回明文） */
const ENV_SPEC: { key: string; label: string; hint?: string }[] = [
  ...
  { key: "FIRECRAWL_API_KEY", label: "Firecrawl API Key", hint: "批量检索缺摘要时兜底抓取" },
  { key: "OPENALEX_EMAIL", label: "OpenAlex 联系邮箱", hint: "批量检索时建议填写，避免限流" },
];
